  ...
  .handler(async ({ data }) => {
    ...
    const { data: jobs } = team
      ? await sb
          ...
          .limit(5)
      : { data: [] as { id: number; status: string; created_at: string }[] };
    return {
...
export const sendToBuild = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: t, error } = await sb
      .from("tenants")
      .select("id, slug, name, legal, phone, email, domain, sites, features, ingest, plan, billing, staff_json, principal_name, group_name, pack_json, stage")
      .eq("id", data.tenantId)
      .maybeSingle();
    ...
    await sb.from("tenants").update({
      stage: "build",
      pack_json: JSON.stringify(pack),
      repo_slug: `desk-${pack.slug}`,
    }).eq("id", data.tenantId);
    const { error: jobErr } = await sb.from("build_jobs").insert({
      tenant_id: data.tenantId,
      status: "queued",
    ...
    await sb.from("build_events").insert({
      ...
      stage: "build",
      title: "Sent to build",
      body: "The pack is locked. Desk is being stood up from the template — not from Aberdeen.",
      visibility: "customer",
      actor_email: email,
    });
    return { ok: true, brief, repo: `desk-${pack.slug}` };
  });
